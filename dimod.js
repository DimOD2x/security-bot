...
const dimod = new Client({ disableEveryone: true });
...
///////////////////////////////////////////////////////////////////////////////
dimod.on("ready", () => {
  console.log(`${dimod.user.tag}`);
  dimod.user.setActivity(`${prefix}help`, {
    Type: "Playing"
  });
});
///////////////////////////////////////////////////////////////////////////////
dimod.on("message", async message => {
...
**Links**
[Support](https://discord.gg/msrPc36PSX) - [Invite](https://discord.com/api/oauth2/authorize?client_id=${dimod.user.id}&permissions=8&scope=bot) - [Website](https://security-bot4415.glitch.me/)
...
dimod.on("message", message => {
  ...
  if (cooldown.has(message.author.id)) {
    ...
    let embed = new Discord.MessageEmbed()
      .setColor(dimodcolor)
      .setImage(`dimodlogo`)
      .setThumbnail(dimod.user.avatarURL())
      .setDescription(`
**General Rules 
...
dimod.on("message", async message => {
  if (message.content.startsWith(prefix + "invite")) {
    ...
    let help = new Discord.MessageEmbed()
      .setColor(callicolor)
      .setDescription(`
[Click here](https://discord.com/api/oauth2/authorize?client_id=884137061282639942&permissions=0&scope=bot) **to invite the bot.**
`);
    message.channel.send(help);
  }
});
...
dimod.on("message", message => {
  if (message.content.startsWith(`${prefix}botinfo`)) {
    ...
    const tnx = new Discord.MessageEmbed()
      .setColor(callicolor)
      .addField("Name", `${dimod.user.tag}`, true)
      .addField("Name", `${dimod.user.tag}`, true)
      .addField("ID", `${dimod.user.id}`, true)
      .addField("Version", `${process.version}`, true)
      .addField("Guilds", `${dimod.guilds.cache.size} Guilds`, true)
      .addField("Users", `${dimod.users.cache.size} Users`, true)
      .addField(
        "Ping",
...
dimod.on("message", message => {
  if (message.content === prefix + "servers") {
    ...
    let embed = new Discord.MessageEmbed()
      .setColor(callicolor)
      .addField("Guilds", `${dimod.guilds.cache.size} Guilds`, true)
      .addField("Users", `${dimod.guilds.cache.reduce((a, g) => a + g.memberCount, 0)} Users`, true)
      .setThumbnail(message.member.user.displayAvatarURL({ dynamic: true }));
    message.channel.send({ embed });
  }
});
...
dimod.on("message", async message => {
  ...
  if (["ban", "kick"].includes(commandName)) {
    ...
    let bot = message.guild.member(dimod.user);
    if (user.user.id == dimod.user.id) return message.channel.send("lol no");
...
dimod.on("message", message => {
  ...
  if (commands[0] == prefix + "embed") {
    ...
    if (!message.guild.member(dimod.user).hasPermission("MANAGE_MESSAGES"))
...
//////////////////////////////////////////////////////////////////////////////
dimod.on("message", async message => {
  if (message.content.startsWith(`<@${dimod.user.id}>`)) {
    if (cooldown.has(message.author.id)) {
      return message.channel.send(`You have to wait 5 seconds`).then(m => {
        m.delete({ timeout: cdtime * 600 });
      });
    ...
    let help = new Discord.MessageEmbed()
      .setColor(dimodcolor)
      .setAuthor(dimod.user.username, dimod.user.avatarURL())
      .setThumbnail(dimod.user.avatarURL())
...
dimod.on("channelCreate", async channel => {
...
dimod.on("channelDelete", async channel => {
  const entry1 = await channel.guild
    .fetchAuditLogs({
      type: "CHANNEL_DELETE"
    })
...
dimod.on("roleDelete", async channel => {
  const entry1 = await channel.guild
    .fetchAuditLogs({
      type: "ROLE_DELETE"
    })
...
dimod.on("roleCreate", async channel => {
  const entry1 = await channel.guild
    .fetchAuditLogs({
      type: "ROLE_CREATE"
    })
...
dimod.on("guildBanAdd", async (guild, user) => {
...
dimod.on("guildKickAdd", async (guild, user) => {
  const entry1 = await guild
    .fetchAuditLogs({
      type: "MEMBER_KICK"
    })
...
dimod.on("guildMemberRemove", async member => {
